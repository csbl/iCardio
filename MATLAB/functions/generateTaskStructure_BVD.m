function taskStruct = generateTaskStructure_BVD(inputFile)
...
columns={'ID';'DESCRIPTION';'IN';'IN LB';'IN UB';'OUT';'OUT LB';'OUT UB';'SHOULD FAIL';'COMP';'SYSTEM';'SUBSYSTEM';'EQU';'EQU LB';'EQU UB'};
...
for i=2:size(raw,1)
    ...
    %Set the equations
    if ischar(raw{i,colI(13)})
        task.EQU=[task.EQU;raw{i,13}];
        task.EQULB=[task.EQULB;raw{i,colI(14)}];
        task.EQUUB=[task.EQUUB;raw{i,colI(15)}];
    end
