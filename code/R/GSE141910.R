...
dir <- "F:/Transcriptomics/kallisto/outputs/GSE141910"

# Check for all files in the directory to ensure they are present 
list.files(dir)

# Read in meta data table
metadata <- read.xlsx(file.path("F:/Transcriptomics/kallisto/outputs/GSE141910/metadata.xlsx"), header = TRUE, sheetIndex =1) %>% 
...
condition = metadata$condition.2
...
metadata %>% group_by(condition.2) %>% dplyr::count()

keep <- rowSums(counts(dds) >= 10) >= 162
dds <- dds[keep,]
nrow(dds)
...
failure_DEGs <- data.frame(DESeq2::results(dds, 
                                          contrast = c("condition", "failure", "healthy"), 
                                          alpha = 0.1, 
                                          pAdjustMethod = "BH", 
...
write.table(all.results, file = "C:/Users/bvd5nq/Documents/R scripts/Cardiomyocyte Model/GSEdata/GSE141910_failure_DEGs.csv", 
...
write.table(model.genes.data, file = "C:/Users/bvd5nq/Documents/R scripts/Cardiomyocyte Model/GSEdata/GSE141910_failure_model_DEGs.csv", 
            col.names = TRUE, row.names = FALSE, sep = ",")
